...
for (let i = 0; i < data.length; i++) {
  ...
  for (let j = 0; j < data.length; j++) {
    ...
    //so sánh từng features của data[i] với data[j]
    ...
    column.push(similarities / features); //tương quan giống nhau giữa data[i] và data[j]
  }
  //kết quả data[i] so với data[j]
  matrix.push(column);
}
//kết quả ma trận
// console.log(matrix);
